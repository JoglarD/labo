...
PARAM  <- list()
PARAM$experimento  <- "CA6110ESTADISTICO"
...
CorregirCampoMes <- function(pcampo, pmeses) { 
  tbl <- dataset[, .(v1 = shift(get(pcampo), 1:3, type = "lag"),
                     v2 = shift(get(pcampo), 1:3, type = "lead")), 
                 by = numero_de_cliente] 
  
  tbl[, numero_de_cliente := NULL]
  tbl[, mediana := apply(tbl[, -1, with = FALSE], 1, function(x) median(as.numeric(unlist(x)), na.rm = TRUE))]
  dataset[, 
...
----------------------------------------------------------------------------
# reemplaza cada variable ROTA  (variable, foto_mes)  con la mediana entre  ( los 3 meses_anteriores, 3 mes_posteriores )

Corregir_EstadisticaClasica  <- function( dt )
{
